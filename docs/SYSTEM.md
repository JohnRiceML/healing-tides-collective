...
| **Phase 2 — Get Matched (seeker side)** | Conversational onboarding agent (text + voice) → intake → Nora's matching workspace → shortlist delivery; optional seeker accounts | 🟢 **Live (prod)** — the guided core is real now, not a prototype (the old `/prototype/*` mock tree was deleted 2026-06-28; it lives in git history) |
...
- **Honest caveats the surfaces table can hide** (don't over-claim): (1) the matching **workspace is a curation tool, not a ranker** — no fit score; structured matching fields are deferred (ADR [0001](decisions/0001-matching-workspace-curation-not-ranker.md)). (2) **Email is wired (Resend) but dormant until keyed** — `lib/email.ts` no-ops without `RESEND_API_KEY`+`EMAIL_FROM`, so claim invites, the seeker welcome email, and the **new-intake notification to admins** (`lib/seeker-notify.ts` — fires on every seekerIntake create, recipients = the `ADMIN_EMAILS` allowlist) mint/queue but don't *send* until John sets them. (3) **Real-seeker launch is HIPAA-gated** — accounts are built minimal + anonymous-by-default, but pointing actual clients at intake/accounts waits on Christie's determination. (4) the old **`/prototype/*` mock tree was deleted** 2026-06-28 (superseded by the real surfaces; recover from git history if ever needed).
